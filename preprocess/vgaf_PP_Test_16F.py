...
for (root, dirs, files) in os.walk(test_videos_path): #  train_videos_path - CHANGE -
  for file in files:
...
for video_id in range(len(video_list)):
  ...
  with open(test_videos_frames_path+'/'+str(video_name)+'.pkl','wb') as f: # val_videos_frames_path - CHANGE
